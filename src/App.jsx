...
function App() {
  ...
  const [checked, setChecked] = useState(Boolean(JSON.parse(localStorage.getItem('checkedDarkMode'))));
  ...
  const MaterialUISwitch = styled(Switch)(({ theme }) => ({
    ...
    '& .MuiSwitch-thumb': {
      backgroundColor: mode === 'dark' ? '#009688' : '#001e3c',
      width: 32,
      height: 32,
      '&:before': {
        content: "' '",
  ...
  const hanldeChangeDarkMode = () => {
    setChecked((prev) => {
      ...
      //  lưu chế độ sảng tối => vào localstorage
      localStorage.setItem('checkedDarkMode', JSON.stringify(newChecked));
      return newChecked;
    });
  ...
  return (
    <>
      ...
      <div>
        {/* dark mode theo chế icon switch */}
      ...
      <Typography variant="h6" color="text.primary3">
        {' '}
        Quang Hải Tiki 4
      </Typography>
      <Button
