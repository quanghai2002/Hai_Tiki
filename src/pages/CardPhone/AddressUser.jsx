...
function AddressUser({ isModal, setIsModalOpenAddress, setAddressUserShip }) {
  const [loading, setLoading] = useState(false);
  const dispatch = useDispatch();
  // ---- ----------KIỂM TRA XEM ĐÃ CÓ ĐỊA CHỈ GIAO HÀNG HAY CHƯA -------------------
  const userLogin = useSelector((state) => state?.userAuth?.user);

  const [isUser, setIsUser] = useState(Boolean(userLogin?.address));
  ...
  const {
    ...
    defaultValues: {
      nameUser: isUser ? userLogin?.username : '',
      phoneNumber: isUser ? userLogin?.phoneNumber : '',
    },
  });
  ...
  const onSubmit = async (data) => {
    ...
    // console.log('dữ liệu chuẩn bị cập nhật User', dataUpdateUser);
    // ----CẬP NHẬT THÔNG TIN USER LÊN SERVER ----
    userApi
      .updateOneUser(dataUpdateUser)
      .then((response) => {
        // console.log('cập nhật thông tin user thành công', response);
        ...
        setLoading(false);
        // set lại modal bằng false để đóng modal
        setIsModalOpen(false);
        // cập nhật vào trong redux
        dispatch(updateUser(response?.data));

        // set lại isAddres ở trang trước để hiển thị thông tin user cập nhật ra màn hình
        setAddressUserShip(true);
      })
      .catch((err) => {
  ...
  return (
    <Box className={clsx(style.wrapAddressUser)}>
      ...
      <Box className={clsx(style.noAddress)}>
        ...
        >
          {/* rect hook from */}
          {/* khi đang cập nhật Thông Tin address => Spin */}
          {loading ? (
            <Spin>
