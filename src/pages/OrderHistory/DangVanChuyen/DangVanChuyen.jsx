...
function DangVanChuyen({ keyTab, listAllOrder, loading }) {
  ...
  // đây là để TEST khi đơn hàng trống => Không có đơn hàng ở trạng thái -- ĐANG VẬN CHUYỂN --
  // const dataTest = [];

  // LỌC RA CÁC ĐƠN HÀNG CÓ TRẠNG THÁI 2 => ĐANG VẬN CHUYỂN
  const DataSort = dataTest?.filter((item) => {
    return item?.trangthai.code === 2;
  });
  ...
  //  --- RETURN JSX --
  // --KHI ĐANG CALL API HIỂN THỊ LOADING RA ---
  return loading ? (
    <LazyOrder />
  ) : (
    <Box className={clsx(style.wrapAllDangVanChuyen)}>
      ...
      {DataSort?.length > 0 ? (
        DataSort?.map((item) => {
          return (
            <Box className={clsx(style.dangvanchuyen)} key={`${item?.id} ${Math.random() * 11000}`}>
              {/* header */}
