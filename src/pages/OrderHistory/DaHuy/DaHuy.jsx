...
import style from './DaHuy.module.scss';
...
function DaHuy({ keyTab, listAllOrder, loading }) {
  ...
  // LỌC RA CÁC ĐƠN HÀNG CÓ TRẠNG THÁI 4 => ĐÃ HỦY
  const DataSort = dataTest?.filter((item) => {
    return item?.trangthai.code === 4;
  });
  ...
  return loading ? (
    ...
    <Box className={clsx(style.wrapAllDaHuy)}>
      ...
      {DataSort?.length > 0 ? (
        DataSort?.map((item) => {
          return (
            <Box className={clsx(style.dahuy)} key={`${item?.id} ${Math.random() * 9000}`}>
              {/* header */}
